...
async def daily_fetch_job(bot: Bot, chat_id: str) -> None:
    """08:00 UTC — fetch 5 new papers, score, store, broadcast high-scoring ones."""
...
def setup_scheduler(bot: Bot, chat_id: str) -> AsyncIOScheduler:
    ...
    # Daily paper fetch — every day at 08:00 UTC
    scheduler.add_job(
        daily_fetch_job,
        trigger="cron",
        hour=8,
    ...
    # Weekly digest — every Sunday at 09:00 UTC (1 hour after daily fetch)
    scheduler.add_job(
        weekly_digest_job,
        trigger="cron",
        day_of_week="sun",
